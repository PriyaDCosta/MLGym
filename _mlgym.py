...
# CONFIG_DIR = "/Users/priyadcosta/Documents/MLGym/configs"
try:
    import rich
...
class MLGym:
    ...
    def get_args(self) -> ScriptArguments:
        ...
        container_type = self.configs["container_type"]
        task_config_path = self.configs["task_config_path"]
        model_name = self.configs["model_name"]
        per_instance_cost_limit = float(self.configs["per_instance_cost_limit"])
        agent_config_path = self.configs["agent_config_path"]
        temperature = int(self.configs["temperature"])
        gpus = int(self.configs["gpus"])
        max_steps = int(self.configs["max_steps"])
        aliases_file = self.configs["aliases_file"]

        args = [
            "--container_type", self.configs["container_type"],
            "--task_config_path", self.configs["task_config_path"],
            "--model_name", self.configs["model_name"],
            "--per_instance_cost_limit", self.configs["per_instance_cost_limit"],
            "--agent_config_path", self.configs["agent_config_path"],
            "--temperature", self.configs["temperature"],
            "--gpus", self.configs["gpus"],
        ...
        defaults = ScriptArguments(
            environment=EnvironmentArguments(
                task_config_path=task_config_path, max_steps=max_steps, seed=42, container_type=container_type, verbose=True
            ),
            agent=AgentArguments(
    ...
    def execute(self):
        ...
        try:
            ...
            if "asyncio.run() cannot be called from a running event loop" in str(e):
                ...
                loop.run_until_complete(Main(args).main())
            else:
                raise
